(function() {
  ...
  define(['jquery', 'underscore', 'compiled/calendar/commonEventFactory', 'jquery.ajaxJSON', 'vendor/jquery.ba-tinypubsub'], function($, _, commonEventFactory) {
    return (function() {
      ...
      _Class.prototype.getEvents = function(start, end, contexts, cb, options) {
        var dataCB, doneCB, list, params, paramsForDatedEvents, paramsForUndatedEvents, requestResults,
          _this = this;
        ...
        if (!params) {
          list = this.getEventsFromCache(start, end, contexts);
        ...
        requestResults = {};
        dataCB = function(data, url, params) {
          var e, event, key, requestResult, _i, _len;

          if (!data) {
            return;
          }
          key = 'type_' + params.type;
          requestResult = requestResults[key] || {
            events: []
          };
          ...
          requestResult.maxDate = _.max(requestResult.events, function(e) {
            return e.originalStart;
          }).originalStart;
          return requestResults[key] = requestResult;
        };
        doneCB = function() {
          var context, contextInfo, event, incomplete, key, lastKnownDate, nextPageDate, requestResult, _i, _j, _len, _len1, _ref;

          incomplete = false;
          for (key in requestResults) {
            requestResult = requestResults[key];
          ...
          if (nextPageDate) {
            nextPageDate.setHours(0, 0, 0);
          }
          lastKnownDate = start;
          for (key in requestResults) {
            requestResult = requestResults[key];
            _ref = requestResult.events;
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              event = _ref[_i];
              if (!incomplete || event.originalStart < nextPageDate) {
                _this.addEventToCache(event);
                if (event.originalStart > lastKnownDate) {
                  lastKnownDate = event.originalStart;
                }
              }
            }
          }
          ...
          for (_j = 0, _len1 = contexts.length; _j < _len1; _j++) {
            context = contexts[_j];
            contextInfo = _this.cache.contexts[context];
            if (contextInfo) {
              if (start) {
          ...
          list = _this.getEventsFromCache(start, lastKnownDate, contexts);
          list.nextPageDate = incomplete ? nextPageDate : null;
          list.requestID = options.requestID;
          return cb(list);
        };
      ...
      _Class.prototype.startFetch = function(urlAndParamsArray, dataCB, doneCB, options) {
        var inFlightCheckKey, numCompleted, urlAndParams, wrapperCB, _i, _len, _results,
          _this = this;
      ...
      _Class.prototype.fetchNextBatch = function(url, params, cb, options) {
        var parseLinkHeader,
          _this = this;
        ...
        return $.ajaxJSON(url, 'GET', params, function(data, xhr) {
          ...
          if ((rels != null ? rels.next : void 0) && !options.singlePage) {
            cb(data, false);
