(function() {
  ...
  define(['i18n!submission_details_dialog', 'jquery', 'jst/SubmissionDetailsDialog', 'compiled/gradebook2/Turnitin', 'jst/_submission_detail', 'jst/_turnitinScore', 'jquery.ajaxJSON', 'jquery.disableWhileLoading', 'jquery.instructure_forms', 'jqueryui/dialog', 'jquery.instructure_misc_plugins', 'vendor/jquery.scrollTo', 'vendor/jquery.ba-tinypubsub'], function(I18n, $, submissionDetailsDialog, _arg) {
    ...
    return SubmissionDetailsDialog = (function() {
      function SubmissionDetailsDialog(assignment, student, options) {
        ...
        }).delegate('.submission_details_grade_form', 'submit', function(event) {
          event.preventDefault();
          return $(event.currentTarget.form).disableWhileLoading($.ajaxJSON(_this.url, 'PUT', $(event.currentTarget).getFormData(), function(data) {
            _this.update(data);
            $.publish('submissions_updated', [_this.submission.all_submissions]);
      ...
      SubmissionDetailsDialog.prototype.open = function() {
        ...
        return $('.submission_details_dialog .assignment-name').focus();
      };
