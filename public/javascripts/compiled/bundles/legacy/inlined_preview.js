(function() {
  require(["jquery", "jquery.instructure_misc_plugins", "jquery.templateData"], function($) {
    return $(document).ready(function() {
      ...
      $(window).resize(function() {
        ...
        return $("#modules").height(height - top);
      }).triggerHandler("resize");
      maxWidth = 0;
      $(".context_module_item").each(function() {
        return maxWidth = Math.max(maxWidth, $(this).width());
      });
      ...
      $("#modules .context_module_item.attachment a.title,#modules .context_module_item.external_url a.title").each(function() {
        return $(this).attr("href", $(this).attr("href") + "?already_inline=1");
      });
      $("#modules .context_module_item").filter(".attachment,.external_url").find("a.title").each(function() {
        return $(this).attr("target", "content_preview");
      }).click(function(event) {
        var id;

        event.preventDefault();
        $("#file_display_name").text($(this).text());
        id = $(this).parents(".context_module_item").getTemplateData({
          textValues: ["id"]
        }).id;
        return location.replace("#tag_" + id);
      });
      $(document).fragmentChange(function(event, hash) {
        ...
        if (hash.match(/^#tag_/)) {
          ...
          if (id && id !== $("#current_item_id").text() && $("#context_module_item_" + id + " a.title").length > 0) {
            ...
            return $("#context_module_item_" + id + " a.title").click();
          }
        }
      });
      ...
      $("#frameless_link").click(function(event) {
        event.preventDefault();
        return location.href = $("#content_preview").attr("src");
      });
      $(".hide_sidebar_link").click(function(event) {
        event.preventDefault();
        return $("#modules").parent().hide();
      });
