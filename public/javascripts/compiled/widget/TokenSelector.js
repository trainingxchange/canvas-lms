  ...
  define(['jquery', 'underscore', 'compiled/widget/TokenSelectorList', 'compiled/collections/RecipientCollection', 'jquery.instructure_misc_helpers', 'compiled/jquery/scrollIntoView'], function($, _, TokenSelectorList, RecipientCollection) {
    return (function() {
      ...
      _Class.prototype.selectNext = function(preserveMode) {
        ...
        this.select(this.selection ? this.selection.next().length ? this.selection.next() : this.selection.parent('ul').next().length ? this.selection.parent('ul').next().find('li').first() : null : (_ref = this.list) != null ? _ref.first() : void 0, preserveMode);
      ...
      _Class.prototype.selectPrev = function() {
        ...
        this.select(this.selection ? ((_ref = this.selection) != null ? _ref.prev().length : void 0) ? this.selection.prev() : this.selection.parent('ul').prev().length ? this.selection.parent('ul').prev().find('li').last() : null : (_ref1 = this.list) != null ? _ref1.last() : void 0);
      ...
      _Class.prototype.updateSearch = function() {
        ...
        return this.timeout = setTimeout(function() {
          ...
          if (_this.lastFetch && !_this.lastFetch.isResolved()) {
            _this.nextRequest = true;
            return;
          }
      ...
      _Class.prototype.lastFetch = null;

      _Class.prototype.collectionForQuery = function(query) {
        var cacheKey, collection, _ref;

        if ((_ref = this.lastFetch) != null) {
          _ref.abort();
        }
        ...
        if (this.cache[cacheKey] == null) {
          ...
          this.lastFetch = collection.fetch({
            data: query
          });
      ...
      _Class.prototype.listForQuery = function(query) {
        ...
        if (!collection.atLeastOnePageFetched) {
          collection.on('fetch', _.once(function() {
            _this.autoSelectFirst(list);
            if (_this.nextRequest) {
              _this.updateSearch();
            }
            return delete _this.nextRequest;
          }));
        }
        return list;
      };
