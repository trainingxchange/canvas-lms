(function() {
  define(['jquery', 'compiled/util/objectCollection', 'jst/courseList/wrapper', 'jst/courseList/content', 'jquery.ajaxJSON'], function(jQuery, objectCollection, wrapper, content) {
    ...
    return CustomList = (function() {
      ...
      CustomList.prototype.open = function(e) {
        var _this = this;
        ...
        setTimeout(function() {
          ...
          if (typeof (_base = _this.options).onToggle === "function") {
            _base.onToggle(true);
          }
          document.activeElement.blur();
          return _this.wrapper.find('li a').first().focus();
        }, 1);
        return this.element.find('.customListOpen').attr({
          'aria-expanded': 'true'
        });
      };

      CustomList.prototype.close = function(e) {
        var _base;
        ...
        document.activeElement.blur();
        return this.element.find('.customListOpen').focus().attr({
          'aria-expanded': 'false'
        });
      };
      ...
      CustomList.prototype.attach = function() {
        ...
        this.wrapper.delegate('.customListItem', 'click.customListTeardown', jQuery.proxy(this, 'sourceClickHandler'));
        id = 'customListWrapper-' + jQuery.guid++;
        this.wrapper.appendTo(this.appendTarget).attr({
          id: id
        }).hide();
        return this.element.find('.customListOpen').attr({
          role: 'button',
      ...
      CustomList.prototype.setOn = function(element, bool) {
        element.toggleClass('on', bool);
        return element.find('a').attr('aria-checked', bool.toString());
      };
      ...
      CustomList.prototype.setPinned = function() {
        ...
        this.setOn(this.wrapper.find('ul > li'), false);
        _ref = this.pinned;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          item = _ref[_i];
          match = this.wrapper.find("ul > li[data-id=" + item.id + "]");
          _results.push(this.setOn(match, true));
        }
        return _results;
      };
